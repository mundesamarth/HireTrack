...
export default function ModalForm({ setIsModalOpen, fetchData }: Props) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  async function submitModalApplication(data: Inputs) {
    try {
      ...
      gooeyToast.success("Application added successfully", {
        description: "New Job applications have been added to the board.",
        preset: "smooth",
      });
    } catch (error) {
      console.log("Something went wrong in create application",error);
  ...
  const onSubmit: SubmitHandler<Inputs> = submitModalApplication;
    ...
    <div>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="pt-10 flex justify-end gap-4">
          ...
          <Button
            className="px-10 py-6 bg-foreground-1 text-background cursor-pointer hover:bg-foreground-2 font-[700]"
            type="submit"
          >
            + Add Job
          </Button>
